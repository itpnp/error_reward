...
class tilang extends CI_Controller {
	...
	public function registerTicket($id){
		...
		if($session!=""){
			...
			$data["jumlahTilang"] = $this->M_tilang->countTotalTicket($id,date('m'),date('Y'));
			$data["dendaTilang"] = 0;
			$nominal = $this->M_nominal->findByJabatan($data["dataKaryawan"]->occupation->occupationId);
			if($data["jumlahTilang"]==0){
				$data["dendaTilang"] = $nominal->tilang1;
			}else if($data["jumlahTilang"]==1){
				$data["dendaTilang"] = $nominal->tilang2;
			}else if($data["jumlahTilang"]==2){
				$data["dendaTilang"] = $nominal->tilang3;
			}else if($data["jumlahTilang"]==3){
				$data["dendaTilang"] = $nominal->tilang4;
			}else if($data["jumlahTilang"]==4){
				$data["dendaTilang"] = $nominal->tilang5;
			}
			$this->load->view('Tilang/v_header.php',$data);
			$this->load->view('Tilang/v_sidebar.php',$data);
	...
	public function saveTilang(){
		...
		if($session!=""){
			...
			if($this->M_tilang->savetilang($dataTilang)){
				$this->session->set_flashdata('success', 'Tilang Berhasil Disimpan');
				$this->index();
			}else{
				$this->session->set_flashdata('success', 'Tilang Gagal Disimpan');
				$this->index();
			}
		}else{
			$this->session->set_flashdata('error', 'Anda Masuk Tanpa Login');
